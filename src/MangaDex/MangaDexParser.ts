...
export const parseChapterList = async (chapterObject: any, coversMapping: {[id: string]: string}, source: any, thumbnailSelector: any, ratings: string[]): Promise<MangaTile[]> => {
    ...
    for (const chapter of chapterObject) {
        const mangaId = chapter.data.relationships.filter((x: any) => x.type == 'manga').map((x: any) => x.id)[0]
        const title = chapter.data.relationships.filter((x: any) => x.type == 'manga').map((x: any) => x.attributes.title.en)[0]
        ...
        if (!mangaId || !title || !ratings.includes(chapter.data.relationships.filter((x: any) => x.type == 'manga').map((x: any) => x.attributes.contentRating)[0])) continue
